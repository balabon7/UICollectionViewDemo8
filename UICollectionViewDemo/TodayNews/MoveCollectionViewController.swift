...
class MoveCollectionViewController: UICollectionViewController {
    ...
    fileprivate lazy var collectionDatas: Array = { [
        ["рекомендовать", "горячая точка", "видео", "вопросы и ответы", "физическая культура", "история", "Технологии", "здоровый", "развлечения", "Мода", "история", "прямая трансляция", "цифровой", "опровергнуть слух", "здравоохранение"],
        ["Мейту", "позитивная энергия", "смешной", "культура", "роман", "Цитаты", "Шэньчжэнь", "общество", "автомобиль", "Финансы", "военный", "шутить", "Красота", "интернациональность", "Интересные картинки", "распродажа", "недвижимость", "воспитание детей", "Еда", "Кино"]
        ] }()
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "редактировать", style: .done, target: self, action: #selector(rightItemClick(_:)))
    ...
    @objc func rightItemClick(_ item: UIBarButtonItem) {
        if navigationItem.rightBarButtonItem?.title == "редактировать" {
            navigationItem.rightBarButtonItem?.title = "Заканчивать"
            isEdit = true
...
extension MoveCollectionViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if indexPath.section == 0 {
            headReuseView.titleLabel.text = "мой канал"
            headReuseView.detaiLabel.text = "Перетащите, чтобы отсортировать"
        } else {
            headReuseView.titleLabel.text = "Рекомендация канала"
            headReuseView.detaiLabel.text = "Нажмите, чтобы добавить канал"
        }
